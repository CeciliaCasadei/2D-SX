...
#jid2=$(sbatch -p long --job-name=step2 --parsable run_step2.sh)

#jid3=$(sbatch -p long --job-name=step3 --parsable --dependency=afterok:$jid2 run_step1_3.sh runSetup_3_modelling.py)
#jid3=$(sbatch -p long --job-name=step3 --parsable run_step1_3.sh runSetup_3_modelling.py)

#jid4=$(sbatch -p long  --job-name=step4 --parsable --dependency=afterok:$jid3 run_step4.sh)
jid4=$(sbatch -p long  --job-name=step4 --parsable run_step4.sh)
