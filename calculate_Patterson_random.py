...
def calculate_PattersonFunction(myArguments):
    ...
    IsAtZero_random = numpy.zeros(shape=IsAtZero.shape)
    for i in range(0, IsAtZero.shape[0]):
        IsAtZero_random[i, 0] = IsAtZero[i, 0]
        IsAtZero_random[i, 1] = IsAtZero[i, 1]
        IsAtZero_random[i, 2] = 100*numpy.random.rand(1)[0]
    print IsAtZero
    print IsAtZero_random
    ...
    for n in n_s:
        ...
        for m in m_s:
            ...
            for h in h_s:
                for k in k_s:
                    nMatches = 0
                    for braggRod in IsAtZero_random:
                        hRod = braggRod[0]
                        kRod = braggRod[1]
    ...
    matplotlib.pyplot.savefig('%s/intensitiesAtZero/randomI_real_reciprocalSpace.png'%inputFolder)
    ...
    myFigure.savefig("%s/intensitiesAtZero/randomI_measured_qx_qy_reciprocalLattice.png"%inputFolder, fontsize = 20)
    matplotlib.pyplot.close()
    ...
    myFigure.savefig("%s/intensitiesAtZero/randomI_measured_h_k_reciprocalLattice.png"%inputFolder, fontsize = 20)   
    matplotlib.pyplot.close()
