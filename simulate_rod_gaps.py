...
for indices in rodIndices:
    ...
    for i in numpy.linspace(0.0, 360.0, num=1440, endpoint=False):
        print i
        ...
        for spot in predictedPattern_I:
            ...
            if (h == -hRod and k == -kRod) or (h == hRod+kRod and k == -hRod) or (h == -kRod and k == hRod+kRod):
                ...
                if successFlag == 1:
                    qRods.append(-spot[11])
                    qRods_minus.append(-spot[11])
                qRods_all.append(-spot[11])
                    
    matplotlib.pyplot.figure(facecolor = 'w')
    #(n, bins, patches) = matplotlib.pyplot.hist(qRods, bins=60)
    matplotlib.pyplot.hist(qRods)
    matplotlib.pyplot.savefig('./Output_rodGaps/rod_sampling_distribution_Tilt_%d_rod_%d_%d_detDistance_%d.png'%(tiltAngle_deg, hRod, kRod, detectorDistance*1000))
    matplotlib.pyplot.close()
        
    matplotlib.pyplot.figure(facecolor = 'w')
    matplotlib.pyplot.hist(qRods_plus)
    #(n, bins, patches) = matplotlib.pyplot.hist(qRods_plus, bins=60)
    matplotlib.pyplot.savefig('./Output_rodGaps/rod_sampling_distribution_Tilt_%d_rod_%d_%d_FM_plus_detDistance_%d.png'%(tiltAngle_deg, hRod, kRod, detectorDistance*1000), dpi=4*96)
    matplotlib.pyplot.close()
    
    matplotlib.pyplot.figure(facecolor = 'w')
    matplotlib.pyplot.hist(qRods_minus)
    #(n, bins, patches) = matplotlib.pyplot.hist(qRods_minus, bins=60)
    matplotlib.pyplot.savefig('./Output_rodGaps/rod_sampling_distribution_Tilt_%d_rod_%d_%d_FM_minus_detDistance_%d.png'%(tiltAngle_deg, hRod, kRod, detectorDistance*1000))
    matplotlib.pyplot.close()
    
    matplotlib.pyplot.figure(facecolor = 'w')
    matplotlib.pyplot.hist(qRods_all)
    #(n, bins, patches) = matplotlib.pyplot.hist(qRods_all, bins=60)
    matplotlib.pyplot.savefig('./Output_rodGaps/rod_sampling_distribution_Tilt_%d_rod_%d_%d_noGeometry_detDistance_%d.png'%(tiltAngle_deg, hRod, kRod, detectorDistance*1000))
    matplotlib.pyplot.close()