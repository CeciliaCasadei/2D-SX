...
def plotRodsFunction(myArguments):
    ...
    for indices in rodIndices:
        ...
        try:
            matplotlib.pyplot.close()
            maxI = max(Irod_vector_I_cleaned)
            minI = min(Irod_vector_I_cleaned)
            step = avg_Intensity_I/4
            myBins = numpy.arange(minI-step, maxI+step, step)
            matplotlib.pyplot.figure(facecolor = 'w')
            (n, bins, patches) = matplotlib.pyplot.hist(Irod_vector_I_cleaned, bins=myBins)
            matplotlib.pyplot.axvline(x=0, color='k', linestyle=':')
            matplotlib.pyplot.axvline(x=avg_Intensity_I,                    ymin = 0, ymax = 0.9, color='r')
            matplotlib.pyplot.axvline(x=avg_Intensity_I-stddev_Intensity_I, ymin = 0, ymax = 0.9, color='r', linestyle=':')
            matplotlib.pyplot.axvline(x=avg_Intensity_I+stddev_Intensity_I, ymin = 0, ymax = 0.9, color='r', linestyle=':')
            matplotlib.pyplot.gca().set_xlim([-3*avg_Intensity_I, +5*avg_Intensity_I])
            matplotlib.pyplot.gca().set_ylim([0, +1.2*max(n)])
            matplotlib.pyplot.gca().text(0.03, 0.97, "{(%d, %d)}\n%.1f $\AA$"%(hRod, kRod, resolution), horizontalalignment='left', verticalalignment='top', fontsize=18, transform = matplotlib.pyplot.gca().transAxes) 
            matplotlib.pyplot.gca().text(0.97, 0.97, "(%.1f $\pm$ %.1f) photons"%(avg_Intensity_I, stddev_Intensity_I), horizontalalignment='right', verticalalignment='top', fontsize=18, transform = matplotlib.pyplot.gca().transAxes) 
            matplotlib.pyplot.xlabel(r"I$_{\rm scaled}$ (photons)")
            matplotlib.pyplot.ylabel(r"N$_{\rm measured}$")
            
            matplotlib.pyplot.savefig('%s/Histograms/HIS_r%s_spot_%d_%d.png'%(outputFolder, runNumber, hRod, kRod), dpi = 300, facecolor = 'w')
            matplotlib.pyplot.close()
        except:
            matplotlib.pyplot.close()
            matplotlib.pyplot.figure(facecolor = 'w')
            (n, bins, patches) = matplotlib.pyplot.hist(Irod_vector_I_cleaned)
            matplotlib.pyplot.axvline(x=0, color='k', linestyle=':')
            matplotlib.pyplot.axvline(x=avg_Intensity_I,                    ymin = 0, ymax = 0.9, color='r')
            matplotlib.pyplot.axvline(x=avg_Intensity_I-stddev_Intensity_I, ymin = 0, ymax = 0.9, color='r', linestyle=':')
            matplotlib.pyplot.axvline(x=avg_Intensity_I+stddev_Intensity_I, ymin = 0, ymax = 0.9, color='r', linestyle=':')
        ...
        try:
            ...
            (n, bins, patches) = matplotlib.pyplot.hist(Irod_vector_i_cleaned, bins=myBins)
            matplotlib.pyplot.axvline(x=0, color='k', linestyle=':')
            matplotlib.pyplot.axvline(x=avg_Intensity_i,                    ymin = 0, ymax = 0.9, color='r', linestyle=':')
            matplotlib.pyplot.axvline(x=avg_Intensity_i-stddev_Intensity_i, ymin = 0, ymax = 0.9, color='r', linestyle=':')
            matplotlib.pyplot.axvline(x=avg_Intensity_i+stddev_Intensity_i, ymin = 0, ymax = 0.9, color='r', linestyle=':')
            matplotlib.pyplot.gca().set_xlim([-3*avg_Intensity_i, +5*avg_Intensity_i])
            matplotlib.pyplot.gca().set_ylim([0, +1.2*max(n)])
