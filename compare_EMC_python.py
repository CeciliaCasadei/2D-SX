...
intensityFile_EMC = open('./Cmp_EMC_py/model10_l_equal_zero.hkl', 'r')                                        # h k l I sig(I) nmeas - P3 unmerged - to 7A
...
matplotlib.pyplot.savefig('./Cmp_EMC_py/r0127_Is_python_VS_Is_EMC_7A_logscale.png', dpi=96*4)
...
Ts_py = []
Ts_EMC = []
transformationsFile_python = open('./Cmp_EMC_py/r0127-finalOrientations.txt', 'r')    
transformationsFile_python_list = list(transformationsFile_python)
transformationsFile_EMC = open('./Cmp_EMC_py/model10_l_equal_zero.txt', 'r')                                    
...
for lineNumber in range(0, len(transformationsFile_EMC_list)):
    ...
    try:
        T_py = int(T_py)
        if T_py == 3:
            if T_EMC == 0 or T_EMC == 2 or T_EMC == 4:
                n_good = n_good + 1
            else:
                n_bad = n_bad + 1
        if T_py == 2:
            if T_EMC == 1 or T_EMC == 3 or T_EMC == 5:
                n_good = n_good + 1
            else:
                n_bad = n_bad + 1
        if T_py == 1:
            if T_EMC == 7 or T_EMC == 9 or T_EMC == 11:
                n_good = n_good + 1
            else:
                n_bad = n_bad + 1
        if T_py == 0:
            if T_EMC == 6 or T_EMC == 8 or T_EMC == 10:
                n_good = n_good + 1
            else:
                n_bad = n_bad + 1
    except:
