...
def myParallelFunction(myLattice, unassembledData, 
                       runNumber, imageSelection, latticeSelection, 
                       imageFolder, processingFolder, processingFiguresFolder,
                       bgSubtractionMethod, minimizationMethod,
                       lowResLimit, highResLimit,
                       boxWidth,
                       xGeometry, xGeometry_np,
                       yGeometry, yGeometry_np,
                       distanceThreshold, nIterations,
                       distanceThresholds,
                       nSizeRefSteps, nOrientationRefSteps, nCenterRefSteps,
                       widthSizeRefSteps, widthOrientationRefSteps, widthCenterRefSteps,
    ...
    if processFlag == 1:
        ...
        for predictedPeak in refinedPredictedPattern:
            if predictedPeak[4] <= lowResLimit and predictedPeak[4] >= highResLimit:
                ...
                ### FINAL PLOT ###
                predictedX_vector_i.append(Xpredicted/pixelSize)
                predictedY_vector_i.append(Ypredicted/pixelSize)
                hs.append(predictedPeak[0])
                ks.append(predictedPeak[1])
        ...
        for mySpotKey, mySpot in detectedPeaksDictionary.items():            
            ...
            if hasattr(mySpot, 'xCoM'):
                xCoMs.append(mySpot.xCoM/pixelSize)
                yCoMs.append(mySpot.yCoM/pixelSize)
            ...
            for predictedPeak in myLattice.refinedPredictedPattern:
                if predictedPeak[0] == h and predictedPeak[1] == k:
                    ...
                    predictedX_vector_f.append(refinedX/pixelSize)
                    predictedY_vector_f.append(refinedY/pixelSize)
...
#            myLattice.plotInitialAndFinalPredictedPatternFunction(predictedX_vector_i, predictedY_vector_i, 
#                                                                  predictedX_vector_f, predictedY_vector_f, 
#                                                                  xCoMs, yCoMs, # Shows only detected spots with initial distance from prediction below 8 pxls.
#                                                                  hs, ks,
#                                                                  processingFolder) 
        ### SORTING ###
...
def processing(myArguments):
    ...
    string2 = ' --minimizationMethod <minimizationMethod> --lowResLimit <lowResLimit> --highResLimit <highResLimit>'
    ...
    ### EXTRACT IMAGES ###    
    imageObjectsPath = './Output/ExtractExperimentalInfo/r%s_imagesDictionary.pkl'%runNumber
    fImages = open(imageObjectsPath, 'rb')
    imagesDictionary = pickle.load(fImages)
    fImages.close()
    ...
    unassembledImagesDictionary = {}
    for myKey, myImage in imagesDictionary.items():
#        with h5py.File('%s/%s'%(imageFolder, myImage.fileName), 'r') as f:
#            unassembledData = f['/data/rawdata0'] 
#            unassembledData = numpy.asarray(unassembledData, dtype=numpy.float32)         #### !!!!! ####
#            unassembledImagesDictionary['%s'%myImage.fileName] = unassembledData
        unassembledDataFile = h5py.File('%s/%s'%(imageFolder, myImage.fileName), 'r')
        unassembledData = unassembledDataFile['/data/rawdata0']                       #### int16 #### 
        #unassembledDataFile.close()
        unassembledData = numpy.asarray(unassembledData, dtype=numpy.float32)         #### !!!!! ####
        unassembledDataFile.close()
        unassembledImagesDictionary['%s'%myImage.fileName] = unassembledData
    print 'done'
    ...
    if not imageSelection == '' and not latticeSelection == 0:
        nProcessors = 1        
    joblib.Parallel(n_jobs=nProcessors)(joblib.delayed(myParallelFunction)
                                             (myLattice, unassembledImagesDictionary['%s'%myLattice.fileName], 
                                             runNumber, imageSelection, latticeSelection, 
                                             imageFolder, processingFolder, processingFiguresFolder,
                                             bgSubtractionMethod, minimizationMethod,
                                             lowResLimit, highResLimit,
