...
def refineCellSizeAndOrientation(self, sizeRefinementSteps, orientationRefinementSteps, folderName, RLfolderName):
    ...
    latticeErrorMatrix = numpy.zeros((len(orientationRefinementSteps), len(sizeRefinementSteps)))
    nSizeStep = 0
    for sizeRefinementStep in sizeRefinementSteps: # (21) trial unit cell sizes
        ...
        for trialInPlaneRotation in orientationRefinementSteps: # (21) trial in-plane orientations
            ...
            myPredictedPattern = calculatePredictedPattern.calculatePredictedPatternFunction(reciprocalLattice, trialInPlaneRotation, 
                                                                                             waveVector, tiltAngle, 
                                                                                             self.detectorDistance, self.pixelSize)
            ...
            latticeError, nMatchedPeaks = calculateLatticeErrorForRefinement.calculateLatticeError(myPredictedPattern, indexedPeaksTable)
    ...
    refinedPattern = calculatePredictedPattern.calculatePredictedPatternFunction(refinedRL, refinedOrientation, waveVector, tiltAngle, self.detectorDistance, self.pixelSize)
    
    return {'refinedOrientation': refinedOrientation, 'refinedCellSize': refinedCellSize, 
            'latticeErrorMatrix': latticeErrorMatrix, 'refinedPredictedPattern': refinedPattern,
