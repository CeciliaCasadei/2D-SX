...
def imageSums():
    ...
    for orbit in orbits:
        ...
        # LOOP ON ALL MODULES
        modules_sum = numpy.zeros((2*truncated_halfWidth, 2*truncated_halfWidth))                    # 30x30
...
#                orbitSumMatrix_nonRotated = numpy.zeros(shape = (2*halfWidth, 2*halfWidth))           
#                orbitSumMatrix_nonRotated = numpy.matrix(orbitSumMatrix_nonRotated)
                ...
                try:
                    ...
                    n_x = bgSubtractedOrbitSumMatrix_rotated_normalized.shape[1]
                    n_y = bgSubtractedOrbitSumMatrix_rotated_normalized.shape[0]
                    x = numpy.linspace(0, n_x-1, n_x)
                    y = numpy.linspace(0, n_y-1, n_y)
                    x, y = numpy.meshgrid(x, y)                      # column_index, row_index
                    
                    data = bgSubtractedOrbitSumMatrix_rotated_normalized.ravel()           
                    data = data.T
                    ...
                    initial_guess = (numpy.amax(bgSubtractedOrbitSumMatrix_rotated_normalized), initial_x, initial_y, 2.0, 2.0)
        ...
        # GAUSS FIT
        n_x = bgSubtracted_modules_sum.shape[1]
        n_y = bgSubtracted_modules_sum.shape[0]
        x = numpy.linspace(0, n_x-1, n_x)
        y = numpy.linspace(0, n_y-1, n_y)
        x, y = numpy.meshgrid(x, y)                      # column_index, row_index
        
        data = bgSubtracted_modules_sum.ravel()           
        data = data.T
        data = numpy.asarray(data)
        data = data.flatten()
        ...
        initial_x = float(n_x)/2
        initial_y = float(n_y)/2
        initial_guess = (numpy.amax(bgSubtracted_modules_sum), initial_x, initial_y, 2.0, 2.0)
        ...
        ### PLOT GAUSS FIT ###
        myFigureObject = matplotlib.pyplot.figure()
        myAxesImageObject = matplotlib.pyplot.imshow(data.reshape(2*truncated_halfWidth, 2*truncated_halfWidth), origin='lower', interpolation='nearest')
        matplotlib.pyplot.gca().contour(x, y, data_fitted.reshape(2*truncated_halfWidth, 2*truncated_halfWidth), 4, colors='w')
