...
def plotPredictedPattern(predictedPattern1, predictedPattern2, resolutionRadii, detectorDistance, wavelength, myString):
    ...
    myAxes.scatter(myX2, myY2, color='r', marker="+", facecolors='none', s=70)
    for label2, x2, y2 in zip(myLabels2, myX2, myY2):
        matplotlib.pyplot.annotate(label2, xy = (x2, y2), xytext = (+3, -3), size = 18, 
    ...
    if not os.path.exists('./patternSymmetry'):
        os.mkdir('./patternSymmetry')   
    myFigure.savefig("./patternSymmetry/%s.png"%myString)
    matplotlib.pyplot.close()
...
inPlaneRotation = 125 * numpy.pi / 180
waveLength = 1.486567
